        ...
        .filter((item) => item.toLowerCase().includes(search.trim().toLowerCase())).map((item, index) => (
            ...
            >
                <Group gap="sm">
                    ...
                    />
                    <span className='text-deepSlate-300'>{item}</span>
                </Group>
            </Combobox.Option>
        ));
        ...
        <Combobox  store={combobox} onOptionSubmit={handleValueSelect} withinPortal={false}>
            <Combobox.DropdownTarget>
                <PillsInput variant='unstyled' size="sm" pointer onClick={() => combobox.toggleDropdown()}
                    leftSection={<div className="bg-deepSlate-100 rounded-full mr-2 text-oceanTeal-400 p-1"><props.icon size={20} /> </div>}
                ...
                >
                    ...
                    <Pill.Group>
                        {value.length > 0 ? (
                            ...
                            <Input.Placeholder className='!text-deepSlate-300'>{props.title}</Input.Placeholder>
                        )}
