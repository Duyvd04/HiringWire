    ...
    return <div data-aos="zoom-out" className="w-2/3 bs-mx:w-full">
        <div className="flex justify-between items-center flex-wrap">
            <div className="flex items-center gap-2">
                <div className="p-3 bg-deepSlate-800 rounded-xl shrink-0 flex ">
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <div className="text-lg text-deepSlate-300 flex flex-wrap xs-mx:text-base"><span>{props.company} &bull; </span><span> {timeAgo(props.postTime||"")} &bull; </span> <span>{props.applicants?props.applicants.length:0} Applicants </span></div>
                </div>
        ...
        <div className="flex justify-between gap-4 sm-mx:flex-wrap">
            {
                card.map((item, index) => <div key={index} className="flex flex-col text-sm gap-1 items-center ">
                    <ActionIcon className="!h-12 !w-12 xs-mx:!h-8 xs-mx:!w-8" variant="light" color="oceanTeal.4" radius="xl" ><item.icon className="h-4/5 w-4/5" /></ActionIcon>
                    <div className="text-deepSlate-300 xs-mx:text-sm">{item.name}</div>
                    <div className="text-base font-semibold xs-mx:text-sm">{props ? props[item.id]:"NA"}{item.id=="packageOffered"&& <> USD</>}</div>
                </div>)}
        </div>
        <Divider size="xs" my="xl" />
        <div>
            ...
            <div className="flex flex-wrap gap-2">
                {
                    props.skillsRequired?.map((skill:any, index:number) => <ActionIcon key={index} className="!h-fit !w-fit font-medium !text-sm xs-mx:!text-xs" variant="light" color="oceanTeal.4" p="xs" radius="xl">{skill}</ActionIcon>)
                }
            </div>
        </div>
        <Divider size="xs" my="xl" />
        <div className="[&>h4]:text-xl [&_*]:text-deepSlate-300 [&_li]:marker:text-oceanTeal-300 [&_li]:mb-1 [&>h4]:text-deepSlate-200 [&>h4]:font-semibold [&>h4]:my-5 [&_p]:text-justify [&_p]:text-sm [&_li]:text-sm" dangerouslySetInnerHTML={{ __html: cleanHTML }}>
        </div>
        <Divider size="xs" my="xl" />
        <div>
