    ...
    return <div className="flex flex-col gap-5">
        {
            ...
            {key!="Website" && <div className="text-sm text-deepSlate-300 text-justify">
                {key!="Specialties"?company[key]:company[key].map((item: string, index: number) => <span key={index}> &bull; {item}</span>)}
                </div>}
            {key=="Website" && <a target="_blank" href={company[key]} className="text-sm text-oceanTeal-400 hover:text-oceanTeal-300">{company[key]}</a>}
        </div> )
        }
