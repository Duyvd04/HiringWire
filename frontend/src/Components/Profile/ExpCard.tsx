...
const ExpCard = (props:any) => {
    ...
    return !edit?<div  data-aos="fade-up" className="flex flex-col gap-2">
        <div className="flex justify-between gap-2 flex-wrap">
            ...
            <div className="text-sm  text-deepSlate-300">{formatDate(props.startDate)} - {props.working?"Present":formatDate(props.endDate)}</div>
        </div>
        <div className="text-sm xs-mx:text-xs text-deepSlate-300 text-justify">
           {props.description}
        </div>
        { props.edit&&<div className="flex gap-5">
            <Button color="oceanTeal.4" onClick={()=>setEdit(true)} variant="outline">Edit</Button>
