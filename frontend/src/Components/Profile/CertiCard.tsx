    ...
    return <div data-aos="zoom-out">
        <div className="flex justify-between sm-mx:flex-wrap">
            <div className="flex gap-2 items-center">
                <div className="p-2 bg-deepSlate-800 rounded-md shrink-0">
                    <img className="h-7" src={`/Icons/${props.issuer}.png`} alt="" />
                </div>
                <div className="flex flex-col">
                    <div className="font-semibold xs-mx:text-sm">{props.name}</div>
                    <div className="text-sm xs-mx:text-xs text-deepSlate-300">{props.issuer}</div>
                </div>
            </div>
            <div className="flex gap-2 ">
                <div className="flex flex-col items-end sm-mx:flex-row sm-mx:gap-2">
                    <div className="text-sm xs-mx:text-xs text-deepSlate-300">Issued {formatDate(props.issueDate)}</div>
                    <div className="text-sm xs-mx:text-xs text-deepSlate-300">ID: {props.certificateId}</div>
                </div>
